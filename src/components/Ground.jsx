...
import { Html, MapControls, OrbitControls, TransformControls } from "@react-three/drei";
...
function Loading() {
    return (
        <Html zIndexRange={[8888888, 9999999]} className="loading" center>
...
export default ({ mt, action, pt, toggle, view }) => {
        ...
        if (action && action.current[0]) {
            if (
                ...
                Object.values(flagTree.current).forEach((flag) => {
                    if (flag.current) flag.current.visible = false;
                });
            ...
            if (
                ...
                Object.values(flagTree.current).forEach((flag) => {
                    if (flag.current) flag.current.visible = true;
                });
                action.current.shift(0);
                return;
            }
            ...
            if (action.current[0].act === "FIND") {
                ...
                action.current.shift(0);
                let match = flags.find((f) => {
                    return (
                        f.key &&
                        f.key.toUpperCase().indexOf(searchKey.toUpperCase()) !== -1
                    );
                });
                if (match) {
                    let des = new Vector3(
                    ...
                    camera.position.lerp(des, 0.5);
                    orb.current.target = des;
    ...
    return (
        <>
            ...
            <Suspense fallback={<Loading />}>{modList}</Suspense>
            <mesh
                ref={rollOverRef}
