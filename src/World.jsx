      ...
      <Suspense fallback={<Loader />}>
        ...
        {weather === "sunny" && (
          <Mod
            ...
            path={`private_models/${assetPrefix}/SKY2.FBX`}
          />
        )}
        {weather === "cloudy" && (
          <Mod
            key={MathUtils.generateUUID()}
            path={`private_models/${assetPrefix}/SKY1.FBX`}
          />
        )}
        {tf && (
          <Mod key={MathUtils.generateUUID()} path={`models/event/tf.FBX`} />
        )}
      </Suspense>
      <Ground
        mt={modelTree}
...
const Assets = React.memo(({ pathList, mt, pt }) => {
  return (
    <>
      {pathList.map((path) => {
        return (
          <Mod key={MathUtils.generateUUID()} path={path} mt={mt} pt={pt} />
        );
      })}
    </>
  );
